...
function Sphere3D:contains(point)
...
---@return boolean 如果点在球体表面上则返回true，否则返回false
function Sphere3D:containsPoint(point, tolerance)
    ...
    return math.abs(distanceSquared - radiusSquared) <= tolerance * tolerance
end
...
---@overload fun(self: foundation.shape3D.Sphere3D, eulerX: number, eulerY: number, eulerZ: number): foundation.shape3D.Sphere3D
...
---@param center foundation.math.Vector3 旋转中心点
---@return foundation.shape3D.Sphere3D 自身引用
---@overload fun(self: foundation.shape3D.Sphere3D, quaternion: foundation.math.Quaternion): foundation.shape3D.Sphere3D
...
---@overload fun(self: foundation.shape3D.Sphere3D, scale: foundation.math.Vector3|number): foundation.shape3D.Sphere3D
function Sphere3D:scale(scale, center)
    ...
    self.radius = self.radius * math.sqrt(scaleVec.x * scaleVec.x + scaleVec.y * scaleVec.y + scaleVec.z * scaleVec.z) /
        math.sqrt(3)
    return self
end
...
function Sphere3D:clone()
    return Sphere3D.create(self.center:clone(), self.radius)
end
...
function Sphere3D:projectPoint(point)
    ...
    if distance == 0 then
        return Vector3.create(self.center.x + self.radius, self.center.y, self.center.z)
    end
    return Vector3.create(
        self.center.x + toPoint.x * (self.radius / distance),
        self.center.y + toPoint.y * (self.radius / distance),
        self.center.z + toPoint.z * (self.radius / distance)
    )
end

ffi.metatype("foundation_shape3D_Sphere3D", Sphere3D)
